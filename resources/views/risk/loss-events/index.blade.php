...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">{{ $total }} events recorded &middot; Comprehensive register of all operational loss events</p>
        </div>
        <div class="flex items-center gap-3">
            @can('loss_event.create')
                <a href="{{ route('risk.loss-events.create') }}" class="flex items-center gap-2 px-4 py-2 bg-[#1A365D] text-white rounded-lg text-xs font-semibold hover:bg-[#2D4A7A] transition">
