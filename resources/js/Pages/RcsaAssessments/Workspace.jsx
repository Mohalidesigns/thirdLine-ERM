...
export default function Workspace({
    ...
    const replaceLine = useCallback((row) => {
        setLines((current) => current.map((line) => (line.id === row.id ? row : line)));
    }, []);
    ...
    const save = useCallback(
        (line, changes) => {
            ...
            window.axios
                .patch(route('rcsa.assessments.lines.update', [assessment.id, line.id]), {
                    ...changes,
                ...
                .catch((error) => {
                    ...
                    if (status === 409 || status === 423) {
                        ...
                        // that they lost.
                        if (error.response.data?.line) replaceLine(error.response.data.line);
                        setConflict(error.response.data?.message ?? 'That risk changed while you were working on it.');
                    } else {
                        setConflict('That did not save. Check your connection and try again.');
                        replaceLine(line);
                    }
                })
        ...
        router.post(
            ...
            { line_ids: selected, control_effectiveness: bulkValue },
            {
                ...
                onSuccess: () => {
                    setSelected([]);
                    setBulkValue('');
                    router.reload({ only: ['lines', 'outstanding', 'assessment'] });
                },
            },
        );
    };
    ...
    return (
        <AppLayout
            header={
                <PageHeader
                    ...
                    breadcrumbs={[
                        { label: 'RCSA cycles', href: route('rcsa.cycles.index') },
                        { label: assessment.cycle.name, href: route('rcsa.cycles.show', assessment.cycle.id) },
                                ...
                                {['grid', 'guided'].map((option) => (
                                    <button
                                        ...
                                        className={`rounded px-3 py-1 text-sm capitalize ${
                                            mode === option ? 'bg-[var(--color-primary)] text-white' : 'text-gray-600'
                                        }`}
                                    >
                                        {option}
                                    </button>
        ...
        >
            <Head title={`${assessment.business_unit} — ${assessment.cycle.name}`} />
            ...
            {conflict && (
                <div className="mb-4 flex items-start justify-between gap-4 rounded-lg border border-amber-300 bg-amber-50 p-4">
                    ...
                    <button onClick={() => setConflict(null)} className="text-sm text-amber-700 hover:underline">
                        Dismiss
                    </button>
                </div>
            )}
            ...
            <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
                <div className="min-w-0">
                    ...
                    <div className="mb-3 flex flex-wrap items-center gap-3">
                        ...
                        {selected.length > 0 && editable && (
                            <div className="ml-auto flex items-center gap-2">
                                ...
                                >
                                    <option value="">Set control effectiveness…</option>
                                    {methodology.controlEffectiveness.map((item) => (
                    ...
                    {mode === 'grid' ? (
                        <div className="card">
                            <div className="max-h-[70vh] overflow-auto" ref={gridRef}>
                                <table className="data-table">
                                    <thead className="sticky top-0 z-10 bg-white">
                                        <tr>
                                            <th className="w-8" />
                                            <th className="sticky left-0 z-20 bg-white">Risk</th>
                                            <th className="w-28">Likelihood</th>
                                            <th className="w-28">Impact</th>
                                            <th className="w-20">Inherent</th>
                                            <th className="w-32">Control effectiveness</th>
                                    ...
                                    <tbody>
                                        {visible.map((line, rowIndex) => (
                                            ...
                                            <tr data-row={line.id} className={saving[line.id] ? 'opacity-70' : ''}>
                                                ...
                                                <td>
                                                    <ScaleSelect
                                                        scale={methodology.likelihood}
                                                        value={line.inherent_likelihood}
                                                        disabled={!editable}
                                                        cell={`${rowIndex}:likelihood`}
                                                        onKeyDown={(e) => onCellKeyDown(e, rowIndex, 'likelihood')}
                                                        onChange={(value) => save(line, { inherent_likelihood: value })}
                                                    />
                                                </td>

                                                <td>
                                                    <ScaleSelect
                                                        scale={methodology.impact}
                                                        value={line.inherent_impact}
                                                        disabled={!editable}
                                                        cell={`${rowIndex}:impact`}
                                                        onKeyDown={(e) => onCellKeyDown(e, rowIndex, 'impact')}
                                                        onChange={(value) => save(line, { inherent_impact: value })}
                                                        title={impactTitle(impactCriteria, line.inherent_impact)}
                                                    />
                                                </td>

                                                {/* Computed: greyed and not editable, per §8.2 */}
                                                <td className="bg-gray-50">
                                                    <Badge score={line.inherent_score} level={line.inherent_level} />
                                                </td>
                                                ...
                                                <td>
                                                    <select
                                                        ...
                                                        onKeyDown={(e) => onCellKeyDown(e, rowIndex, 'control')}
                                                        onChange={(e) =>
                                                            save(line, { control_effectiveness: e.target.value || null })
                                                        }
                                                    >
                                                        <option value="">—</option>
                                                        {methodology.controlEffectiveness.map((item) => (
                                                            <option key={item.label} value={item.label} title={item.description}>
                                                                {item.label}
                                                            </option>
                                                        ))}
                                                    </select>
                                                ...
                                                <td className="bg-gray-50">
                                                    <Badge score={line.residual_score} level={line.residual_level} />
                                                </td>
                        ...
                        <GuidedStep
                            ...
                            editable={editable}
                            onChange={(changes) => current && save(current, changes)}
                            onMove={(delta) =>
                ...
                <aside className="space-y-4">
                    <div className="rounded-lg border border-gray-200 bg-white p-4">
                        ...
                        <p className="mt-1 text-2xl font-semibold text-gray-800">{completion}%</p>
                    ...
                    {current && mode === 'guided' && (
                        <HeatPosition line={current} methodology={methodology} />
                    )}
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white p-4">
                        ...
                        {outstanding.issues.length === 0 ? (
                            <p className="mt-2 text-sm text-green-700">
                                Nothing outstanding. Every risk is assessed and every one above appetite has a plan.
                            </p>
                        ) : (
                            <ul className="mt-2 space-y-2">
                                {outstanding.issues.slice(0, 40).map((issue, index) => (
                                    <li key={`${issue.line_id}-${issue.type}-${index}`} className="text-sm">
                                        {/* Click-to-jump, never a generic toast. */}
                                        <button
                                            type="button"
                                            onClick={() => jumpTo(issue.line_id)}
                                            className="text-left text-gray-700 hover:text-[var(--color-primary)]"
                                        >
                                            <span className="font-mono text-xs text-gray-500">{issue.risk_no}</span>{' '}
                                            {issue.message}
                                        </button>
                                    </li>
                                ))}
                                {outstanding.issues.length > 40 && (
                                    <li className="text-xs text-gray-500">
                                        …and {outstanding.issues.length - 40} more.
                                    </li>
                                )}
                            </ul>
                        )}
...
function ScaleSelect({ scale, value, onChange, disabled, cell, onKeyDown, title }) {
    return (
        <select
            className="filter-select w-full"
            disabled={disabled}
...
function Badge({ score, level }) {
    ...
    return (
        <span className={`badge ${LEVEL_TONE[level] ?? 'bg-gray-100 text-gray-700'}`}>
