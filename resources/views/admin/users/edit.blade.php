...
<div class="max-w-3xl mx-auto">
    <!-- Back Navigation -->
    <div class="mb-4">
        <a href="{{ route('admin.users.show', $user) }}" class="text-primary hover:text-primary/80 font-medium inline-flex items-center gap-1 text-sm">
        ...
        <div class="p-6 border-b border-gray-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-0.5">Update information and roles for {{ $user->name }}</p>
                </div>
            </div>
        </div>
                ...
                <div class="border-b border-gray-200 pb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="staff_id" name="staff_id" value="{{ old('staff_id', $user->staff_id) }}" required
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" id="job_title" name="job_title" value="{{ old('job_title', $user->job_title) }}" required
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition">
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select id="business_unit_id" name="business_unit_id" required
                                ...
                                @foreach ($businessUnits as $unit)
                                    <option value="{{ $unit->id }}" {{ old('business_unit_id', $user->business_unit_id) == $unit->id ? 'selected' : '' }}>{{ $unit->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        @foreach($roles as $role)
                            <label class="flex items-start gap-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition {{ in_array($role->name, old('roles', $userRoles)) ? 'bg-primary/5 border-primary/30' : '' }}">
