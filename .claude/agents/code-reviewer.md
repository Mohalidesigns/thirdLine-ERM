...
10. **Database reality.** The suite runs on **MariaDB 10.4** (`phpunit.xml`, `risk_test`) and CI is a single `mariadb:10.4` job, so a green run now does exercise the customer's engine — on `main` it still does not, because `main` keeps the old SQLite config and a `mysql:8.0` CI arm. Raw JSON functions, CTEs, window functions, `information_schema` reads and MySQL-8-only syntax are still rejections: the suite catches them on this branch, but `main` cannot, and portable builder spellings are the correctness requirement `docs/DEVELOPMENT_STANDARD.md` records. A portable builder spelling is not a style preference here — see `CalendarService.php:410`.
