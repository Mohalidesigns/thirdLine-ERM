...
@section('title', 'Edit Entity - GRC Risk Management')
@section('page-section', 'Scoping')
@section('page-title', 'Edit Entity')
...
@section('breadcrumbs')
    ...
    <span class="text-gray-400">/</span>
    <a href="{{ route('risk.scoping.show', $entity) }}" class="text-gray-500 hover:text-[#1A365D]">{{ $entity->entity_code }}</a>
    <span class="text-gray-400">/</span>
...
@section('content')
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-[#1A365D]">Edit Entity: {{ $entity->name }}</h1>
        <p class="text-sm text-gray-500 mt-1">Code: {{ $entity->entity_code }}</p>
    </div>

    <form method="POST" action="{{ route('risk.scoping.update', $entity) }}">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" value="{{ $entity->entity_code }}" disabled
                ...
                <div class="lg:col-span-2">
                    ...
                    <select id="parent_id" name="parent_id"
                        ...
                        @foreach ($parentEntities as $pe)
                            <option value="{{ $pe->id }}" {{ old('parent_id', $entity->parent_id) == $pe->id ? 'selected' : '' }}>
                ...
                <div class="lg:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#1A365D]/20 focus:border-[#1A365D]">{{ old('description', $entity->description) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                    <label for="owner_id" class="block text-sm font-medium text-gray-700 mb-2">Entity Owner</label>
                    <select id="owner_id" name="owner_id"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#1A365D]/20 focus:border-[#1A365D]">
                        <option value="">Select Owner</option>
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" {{ old('owner_id', $entity->owner_id) == $user->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="delegate_owner_id" name="delegate_owner_id"
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" {{ old('delegate_owner_id', $entity->delegate_owner_id) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }} ({{ $user->email }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="flex gap-6">
                        @foreach (['active' => 'Active', 'inactive' => 'Inactive', 'archived' => 'Archived'] as $val => $label)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="status" value="{{ $val }}" {{ old('status', $entity->status) === $val ? 'checked' : '' }}
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
            ...
            @php
                ...
                $currentFrameworks = old('regulatory_frameworks', $entity->regulatory_frameworks ?? []);
            @endphp
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
            ...
            <div class="mb-6">
                ...
                <select id="risk_appetite_level" name="risk_appetite_level"
                    ...
                    @foreach (['averse' => 'Averse', 'minimal' => 'Minimal', 'cautious' => 'Cautious', 'open' => 'Open', 'hungry' => 'Hungry'] as $val => $label)
                        <option value="{{ $val }}" {{ old('risk_appetite_level', $entity->risk_appetite_level) === $val ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                ...
                @php
                    ...
                    $currentCategoryAppetites = old('category_appetites', $entity->category_appetites ?? []);
                @endphp
                        ...
                        <div>
                            ...
                            <select name="category_appetites[{{ $key }}]"
                                ...
                                @foreach (['averse' => 'Averse', 'minimal' => 'Minimal', 'cautious' => 'Cautious', 'open' => 'Open', 'hungry' => 'Hungry'] as $val => $lbl)
                                    <option value="{{ $val }}" {{ ($currentCategoryAppetites[$key] ?? '') === $val ? 'selected' : '' }}>{{ $lbl }}</option>
                                @endforeach
                            </select>
                        </div>
                    @endforeach
        ...
        <div class="flex items-center justify-between">
            <a href="{{ route('risk.scoping.show', $entity) }}" class="px-6 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">
                Cancel
            </a>
