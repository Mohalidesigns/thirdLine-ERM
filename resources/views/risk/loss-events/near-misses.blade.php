...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ url('/risk/loss-events/create?type=near_miss') }}" class="flex items-center gap-2 px-4 py-2 bg-[#1A365D] text-white rounded-lg text-xs font-semibold hover:bg-[#2D4A7A] transition">
    ...
    <x-data-table id="nearMissesTable">
        ...
        @forelse (($nearMissEvents ?? []) as $event)
            <tr>
                ...
                <td class="text-gray-500 text-xs">{{ $event->date_of_discovery?->format('d M Y') ?? '-' }}</td>
                ...
                <td>
                    <div class="flex items-center gap-1">
                        ...
                        @if ($event->status !== 'converted')
                            <form method="POST" action="{{ url('/risk/loss-events/' . $event->id . '/convert') }}" class="inline"
