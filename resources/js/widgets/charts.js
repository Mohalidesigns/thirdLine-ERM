/*
| Chart.js 4 renderers for the widget engine.
|
| Every renderer has the signature (mount, data, envelope, tokens) and returns
| the Chart instance (or null when the payload has nothing to draw — the mount
| is simply left empty so the Blade empty-state shows through).
|
...
export function renderSparkline(canvas, t) {
    Chart.getChart(canvas)?.destroy();

    // Payload sits in a sibling <script data-sparkline-payload>; walk up a
    // few ancestors in case Blade nests the canvas inside a wrapper.
    let script = null;
    let scopeEl = canvas.parentElement;
    for (let hop = 0; scopeEl && !script && hop < 4; hop++) {
        script = scopeEl.querySelector('script[type="application/json"][data-sparkline-payload]');
    ...
    let points;
    try {
        points = JSON.parse(script.textContent);
    } catch {
        return;
    }
    if (!Array.isArray(points) || points.length === 0) return;

    // Fixed-height positioned wrapper, otherwise the responsive resize loop
    // grows the canvas indefinitely (same trap the app.js hook guards).
    const parent = canvas.parentElement;
    if (parent && !parent.dataset.chartWrap && !parent.classList.contains('widget-sparkline')) {
        const wrap = document.createElement('div');
        wrap.className = 'widget-sparkline';
...
function donut(mount, data, env, t) {
    ...
    const narrow = mount.clientWidth > 0 && mount.clientWidth < 360;

    return new Chart(makeCanvas(mount), {
        type: 'doughnut',
        data: {
...
function pareto(mount, data, env, t) {
...
function stackedBarBands(mount, data, env, t) {
...
function groupedBar3(mount, data, env, t) {
...
function bubble(mount, data, env, t) {
...
function stackedArea(mount, data, env, t) {
...
function trendStackedBar(mount, data, env, t) {
...
function cumulativeLine(mount, data, env, t) {
    const periods = data.periods || [];
...
function barByType(mount, data, env, t) {
    const bars = data.bars || [];
    if (bars.length === 0) return null;
...
function tornado(mount, data, env, t) {
...
function lecCurve(mount, data, env, t) {
    if (data.empty === true) return null;
...
function gauge(mount, data, env, t) {
...
function timeline(mount, data, env, t) {
