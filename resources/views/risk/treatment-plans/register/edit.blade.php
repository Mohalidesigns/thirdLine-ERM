...
@section('title', 'Edit ' . ($risk->risk_code ?? 'Risk') . ' - GRC Risk Management')
@section('page-section', 'Risk Register')
@section('page-title', 'Edit ' . ($risk->risk_code ?? 'Risk'))
...
@section('breadcrumbs')
    ...
    <span class="text-gray-400">/</span>
    <a href="{{ route('risk.register.show', $risk) }}" class="hover:text-[#1A365D]">{{ $risk->risk_code }}</a>
    <span class="text-gray-400">/</span>
...
@section('content')
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-[#1A365D]">Edit Risk: {{ $risk->risk_code }}</h1>
                <p class="text-sm text-gray-500 mt-1">{{ $risk->title }}</p>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('risk.register.show', $risk) }}"
                   class="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 flex items-center gap-2 transition-colors">
                    <span class="material-symbols-outlined text-lg">visibility</span> View Risk
                </a>
            </div>
        </div>
    </div>

    <form method="POST" action="{{ route('risk.register.update', $risk) }}" id="riskEditForm">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {{-- Risk Code (read-only) --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Risk Code</label>
                    <input type="text"
                    ...
                    <select id="category_id"
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id ?? $category }}"
                                    {{ old('category_id', $risk->category_id ?? $risk->category) == ($category->id ?? $category) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="risk_source"
                        ...
                        @foreach (['RCSA', 'Audit Finding', 'Incident', 'Regulatory', 'Self-Identified', 'External', 'KRI Breach'] as $source)
                            <option value="{{ $source }}" {{ old('risk_source', $risk->risk_source) === $source ? 'selected' : '' }}>{{ $source }}</option>
                        @endforeach
                    </select>
                    @error('risk_source')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    ...
                    <select id="business_unit_id"
                        ...
                        @foreach ($businessUnits as $unit)
                            <option value="{{ $unit->id ?? $unit }}"
                                    {{ old('business_unit_id', $risk->business_unit_id ?? $risk->business_unit) == ($unit->id ?? $unit) ? 'selected' : '' }}>
                                {{ $unit->name ?? $unit }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="process_id"
                        ...
                        @foreach ($processes as $process)
                            <option value="{{ $process->id ?? $process }}"
                                    {{ old('process_id', $risk->process_id ?? $risk->process) == ($process->id ?? $process) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="status"
                            name="status"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1A365D]/20 focus:border-[#1A365D] @error('status') border-red-500 @enderror"
                            required>
                        @foreach (['Open', 'Mitigating', 'Monitoring', 'Accepted', 'Escalated', 'Closed'] as $status)
                            <option value="{{ strtolower($status) }}"
                                    {{ old('status', $risk->status) === strtolower($status) ? 'selected' : '' }}>
                                {{ $status }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date"
                           ...
                           value="{{ old('date_identified', $risk->date_identified ?? ($risk->created_at ? $risk->created_at->format('Y-m-d') : '')) }}"
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
                ...
                <div class="flex flex-wrap gap-2">
                    @foreach ([1 => 'Rare', 2 => 'Unlikely', 3 => 'Possible', 4 => 'Likely', 5 => 'Almost Certain'] as $score => $label)
                        <label class="cursor-pointer">
                            <input type="radio" name="inherent_likelihood" value="{{ $score }}" class="sr-only peer"
                                   {{ old('inherent_likelihood', $risk->inherent_likelihood) == $score ? 'checked' : '' }} required>
                        ...
                        <div>
                            ...
                            <select name="{{ $field }}"
                                ...
                                @foreach ([1 => 'Insignificant', 2 => 'Minor', 3 => 'Moderate', 4 => 'Major', 5 => 'Catastrophic'] as $score => $impactLabel)
                                    <option value="{{ $score }}" {{ old($field, $risk->$field ?? '') == $score ? 'selected' : '' }}>
                                        {{ $score }}: {{ $impactLabel }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <div class="flex items-baseline gap-2">
                    <span class="text-3xl font-bold text-[#1A365D]" id="inherentScorePreview">{{ $risk->inherent_score ?? 0 }}</span>
                    <span class="text-sm text-gray-600">/25 (Likelihood x Max Impact)</span>
                </div>
                @if ($risk->inherent_rating ?? null)
                    <div class="mt-2">
                ...
                <div>
                    ...
                    <select id="treatment_strategy"
                        ...
                        @foreach (['Mitigate' => 'Reduce likelihood or impact', 'Accept' => 'Accept within appetite', 'Transfer' => 'Insurance or outsourcing', 'Avoid' => 'Exit the risk activity'] as $strategy => $description)
                            <option value="{{ strtolower($strategy) }}"
                                    {{ old('treatment_strategy', $risk->treatment_strategy) === strtolower($strategy) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="risk_velocity"
                        ...
                        @foreach (['Immediate' => '< 1 day', 'Fast' => '1-7 days', 'Moderate' => '1-3 months', 'Slow' => '3-12 months', 'Very Slow' => '> 12 months'] as $velocity => $desc)
                            @php $velocityValue = strtolower(str_replace(' ', '_', $velocity)); @endphp
                            <option value="{{ $velocityValue }}"
                                    {{ old('risk_velocity', $risk->risk_velocity) === $velocityValue ? 'selected' : '' }}>
                                {{ $velocity }} ({{ $desc }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="review_frequency"
                        ...
                        @foreach (['Monthly', 'Quarterly', 'Semi-Annual', 'Annual'] as $freq)
                            @php $freqValue = strtolower(str_replace('-', '_', $freq)); @endphp
                            <option value="{{ $freqValue }}"
                                    {{ old('review_frequency', $risk->review_frequency) === $freqValue ? 'selected' : '' }}>
                                {{ $freq }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="lg:col-span-2">
                    ...
                    @php
                        $existingTags = old('regulatory_tags', $risk->regulatory_tags ?? []);
                        ...
                        @foreach (['CBN ORMS', 'Basel III', 'NDPA', 'NFIU', 'ISO 31000', 'COSO ERM'] as $tag)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox"
                                       ...
                                       {{ in_array($tag, $existingTags) ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number"
                               ...
                               value="{{ old('financial_exposure', $risk->financial_exposure) }}"
                    ...
                    <select id="appetite_category"
                        ...
                        @foreach (['Minimal' => 'Near zero tolerance', 'Conservative' => 'Tightly controlled', 'Moderate' => 'Balanced approach', 'Growth' => 'Calculated risks', 'Aggressive' => 'Higher risk tolerance'] as $level => $desc)
                            <option value="{{ strtolower($level) }}"
                                    {{ old('appetite_category', $risk->appetite_category) === strtolower($level) ? 'selected' : '' }}>
                                {{ $level }} - {{ $desc }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="lg:col-span-2">
                    ...
                    <textarea id="notes"
                              ...
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1A365D]/20 focus:border-[#1A365D]">{{ old('notes', $risk->notes) }}</textarea>
                    @error('notes')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="flex items-center justify-between">
            <div class="flex gap-3">
                ...
                <form method="POST" action="{{ route('risk.register.destroy', $risk) }}"
                      onsubmit="return confirm('Are you sure you want to permanently delete this risk? This action cannot be undone.')"
                      class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
                            class="px-6 py-2 border border-red-300 rounded-lg text-sm font-medium text-red-600 hover:bg-red-50 transition-colors">
                        Delete Risk
                    </button>
                </form>
            </div>
            <div class="flex gap-2">
                ...
                <button type="submit"
                    ...
                    <span class="material-symbols-outlined text-lg">save</span> Update Risk
                </button>
            </div>
        </div>
    </form>
    ...
    <div class="mt-6 text-center">
        <p class="text-xs text-gray-400">
            Created: {{ $risk->created_at ? $risk->created_at->format('M d, Y H:i') : 'N/A' }}
            @if ($risk->updated_at && $risk->updated_at != $risk->created_at)
                | Last Modified: {{ $risk->updated_at->format('M d, Y H:i') }}
            @endif
        </p>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function calculateInherentScore() {
            var likelihood = document.querySelector('input[name="inherent_likelihood"]:checked');
            var impactFields = ['impact_financial', 'impact_operational', 'impact_reputational', 'impact_regulatory'];
            var maxImpact = 0;
            ...
            var likelihoodVal = likelihood ? parseInt(likelihood.value) : 0;
            var score = likelihoodVal * maxImpact;
            var preview = document.getElementById('inherentScorePreview');
            if (preview) {
        ...
        var form = document.getElementById('riskEditForm');
        if (form) {
